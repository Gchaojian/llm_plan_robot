...
VECTOR_STORE = 2  # 0 for Pinecone and 1 for Chroma
if VECTOR_STORE == 0:
    import pinecone
    ...
    # make sure you have FAISS on your local machine `pip install faiss-cpu
...
def init_embedding_model(model_name = 'sensenova/piccolo-base-zh'):
# def init_embedding_model(model_name = 'moka-ai/m3e-base'):
...
def init_vector_store(embedding_model, is_already_indexed=True):
    ...
    if VECTOR_STORE == 0:
        ...
        vector_store = init_faiss_vector_store(embedding_model, is_already_indexed)
    return vector_store
...
def init_faiss_vector_store(embedding_model, is_already_indexed=True):
    """
