...
class PromptLoader:
    ...
    def pri_prompt(self) -> str:
        with open(os.path.join(self.dir_path, "primitives_test.txt")) as f:
            prompt = f.read()
        return prompt

    @property
    def sce_prompt(self) -> str:
        with open(os.path.join(self.dir_path, "scene_test.txt")) as f:
            prompt = f.read()
        return prompt

    @property
    def sys_prompt(self) -> str:
        with open(os.path.join(self.dir_path, "system_test.txt")) as f:
            prompt = f.read()
        return prompt
    
    @property
    def settings_prompt(self) -> str:
        with open(os.path.join(self.dir_path, "task_settings_test.txt")) as f:
            prompt = f.read()
        return prompt
